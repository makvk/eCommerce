...
| Админка `/admin` | CRUD товаров, переходы статусов заказа |
...
- **Картинок у товаров нет** — `ImageUrl` не принимается командами создания/обновления.
  Рисуются градиентные плейсхолдеры по хешу `id`.
- **Каталог грузится целиком** — пагинации в API нет, поиск и сортировка на клиенте.
- **Админка не видит чужие заказы** — эндпоинта «все заказы» нет, показываются только свои.
- **Ошибки логина приезжают как 500** — текст вытаскивается из поля `details`,
  которое заполняется только в DEBUG-сборке.
